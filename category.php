  ...
  ?>
    ...
    <div class="col-md-8"><br />
      <?php 
        ...
        $getSLUG = $_GET['slug'];
        $tampilCAT = mysqli_query($connecDB, "SELECT * FROM category WHERE slug = '$getSLUG' ")or die(mysql_error());
        $C = mysqli_fetch_array($tampilCAT);
        $getIDCAT = $C['idCategory'];
      ?>
                <h3><?php echo $C['category']; ?> Category</h3>
        <?php
          $recent = mysqli_query($connecDB, "SELECT p.*, pc.*, c.* 
                                             FROM post p JOIN postcategory pc 
                                             ON p.idPost=pc.idPost JOIN category c 
                                             ON pc.idCategory=c.idCategory 
                                             WHERE c.idCategory = '$getIDCAT' 
                                             AND p.status = 'posted' 
                                             LIMIT $posisi,$batas");
